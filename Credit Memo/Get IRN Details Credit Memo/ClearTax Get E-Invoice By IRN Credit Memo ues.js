/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Get E-Invoice By IRN CM ues
		Author 			:  	NVT Employee 
		Date            :   17-07-2024
		Description		:	
				...
				if (_logValidation(record_obj)) {
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_irn)) { //if cleartax einvoice irn present then button will display
							...
							form.addButton({
								id: 'custpage_einvoice_by_irn_cm',
								label: 'Get E-Invoice By IRN',
					...
					form.clientScriptModulePath = './ClearTax Get E-Invoice By IRN Credit Memo cli.js';
				}	
			}
		} catch (ex) {
			log.debug({
