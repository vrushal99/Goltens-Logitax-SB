/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Get Decrypt Sign QR Code CM ues
		Author 			:  	NVT Employee 
		Date            :   17-07-2024
		Description		:	
				...
				if (_logValidation(record_obj)) {
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_qr_code)) {
							...
							form.addButton({
								id: 'custpage_einvoice_qr_code_cm',
					...
					form.clientScriptModulePath = './ClearTax Get Decrypted Signed QR Code CM cli.js';
				}	
			}
		} catch (ex) {
			log.debug({
