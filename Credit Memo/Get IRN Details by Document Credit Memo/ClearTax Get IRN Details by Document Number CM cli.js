/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get IRN Details Doc Num CM cli
        Author 			:  	NVT Employee
        Date            :   17-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_IRN_Det_By_Doc() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_get_irn_doc_cm_sui",
                    deploymentId: "customdeploy_cleartax_get_irn_doc_cm_sui"
                });
        ...
        log.error('error in ClearTax_Get_IRN_Det_By_Doc() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_IRN_Det_By_Doc: ClearTax_Get_IRN_Det_By_Doc
}
});