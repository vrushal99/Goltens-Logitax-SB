/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get HSN Details CM cli
        Author 			:  	NVT Employee
        Date            :   17-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_HSN_Details() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_get_hsn_det_cm_sui",
                    deploymentId: "customdeploy_cleartax_get_hsn_det_cm_sui"
                });
        ...
        log.error('error in ClearTax_Get_HSN_Details() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_HSN_Details: ClearTax_Get_HSN_Details
}
});