/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Get HSN Details CM ues
		Author 			:  	NVT Employee 
		Date            :   17-07-2024
		Description		:	
				...
				if (_logValidation(record_obj)) {
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_irn)) { //if cleartax einvoice irn present
							...
							form.addButton({
								id: 'custpage_get_hsn_details',
								label: 'Get HSN Details',
								functionName: "ClearTax_Get_HSN_Details"
							});
					...
					form.clientScriptModulePath = './ClearTax Get HSN Details CM cli.js';
				}
			}
		} catch (ex) {
