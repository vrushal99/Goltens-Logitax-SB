/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get GSTIN Details CM cli
        Author 			:  	NVT Employee
        Date            :   17-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_GSTIN_Details() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_get_gst_det_cm_sui",
                    deploymentId: "customdeploy_cleartax_get_gst_det_cm_sui"
                });
        ...
        log.error('error in ClearTax_Get_GSTIN_Details() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_GSTIN_Details: ClearTax_Get_GSTIN_Details
}
});