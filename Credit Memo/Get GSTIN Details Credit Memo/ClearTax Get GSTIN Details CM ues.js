/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Get GSTIN Details CM ues
		Author 			:  	NVT Employee 
		Date            :   17-07-2024
		Description		:	
				...
				if (_logValidation(record_obj)) {
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_irn)) { //if cleartax einvoice irn present
							...
							form.addButton({
								id: 'custpage_get_gstin_details',
								label: 'Get GSTIN Details',
								functionName: "ClearTax_Get_GSTIN_Details"
							});
					...
					form.clientScriptModulePath = './ClearTax Get GSTIN Details CM cli.js';
				}
			}
		} catch (ex) {
