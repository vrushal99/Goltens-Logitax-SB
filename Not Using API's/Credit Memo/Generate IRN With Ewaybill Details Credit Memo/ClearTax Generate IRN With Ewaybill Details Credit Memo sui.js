/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Generate IRN Ewaybil Det CM sui
        Author 			:  	NVT Employee
        Date            :   13-05-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(['N/record', 'N/config', 'N/search', 'N/http', 'N/xml', 'N/runtime', 'N/ui/dialog', 'N/https', 'N/format', 'N/redirect', './ClearTax Library File.js'], function (record, config, search, http, xml, runtime, dialog, https, format, redirect, ClearTax_Library_File) {
    ...
    function onRequest(context) {
        try {
            ...
            var { subsidiary_obj_gstnum, subsidiary_obj_legalname, subsidiary_obj_name, subsidiary_obj_addr1, subsidiary_obj_addr2, subsidiary_obj_city, subsidiary_obj_zip, state_code_value, subsidiary_obj_addrphone, subsidiary_obj_email } = subsidiaryDetailsForSellerDetails(loadRecord, runtime, config, search);
            ...
            if (response_irn.code == 200) {  //if response code got 200 then this code will execute
                ...
                if (nullCheck(error) && error == "N") {  //if response code got 200 and success got 'N' then this code will execute
                    ...
                    var parseObdy_Irn = parseObdy[0]["govt_response"]["Irn"]
                    var parseObdy_EwbDt = parseObdy[0]["govt_response"]["EwbDt"]
                    if (nullCheck(parseObdy_EwbDt)) {

                        // var dateformat = new Date(String(parseObdy_EwbDt))

                        // var formattedTime = format.parse({
                        //     value: parseObdy_EwbDt,
    ...
    function subsidiaryDetailsForSellerDetails(loadRecord, runtime, config, search) {
