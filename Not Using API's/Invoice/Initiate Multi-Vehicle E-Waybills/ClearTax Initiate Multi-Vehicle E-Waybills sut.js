			...
			if (context.request.method === 'GET') {
				...
				try {
					...
					if (nullCheck(transportMode)) {
						...
						var invoiceDocNo = Invoice_obj.getValue({
							fieldId: 'tranid'
						})
						...
						if (response.code == 200) {  //if response code got 200 then this code will execute
							...
							if (nullCheck(responseBodyParse.errors) || nullCheck(responseBodyParse.error_message)) { //if errors is coming in response then set only request and response on selected invoices
								...
								if (nullCheck(responseBodyParse.errors)) {
									...
									var resmsg = 'Multi-Vehicle E-Waybill Not Initiated for This Invoice - ' + JSON.stringify(invoiceDocNo) + '. Error is - ' + JSON.stringify(responseBodyParse.errors[0].error_message);
		
								}
								if (nullCheck(responseBodyParse.error_message)) {
		
									var resmsg = 'Multi-Vehicle E-Waybill Not Initiated for This Invoice - ' + JSON.stringify(invoiceDocNo) + '. Error is - ' + JSON.stringify(responseBodyParse.error_message);
								}
		
								var invURL = 'https://' + accountHost + "/app/accounting/transactions/custinvc.nl?id=" + invoiceId + "&compid=" + accountId + "&whence=";

								var tableMarkup = '<p style="text-align:left; font-size:20px;">Error: ' + resmsg + '</p>';
								tableMarkup += '<p style="text-align:left; font-size:15px;">Navigate To Invoice Record. -> Please <a href= ' + invURL + '>Click Here</p>';
								...
								var resmsg = 'Multi-Vehicle E-Waybill Initiated for This Invoice - ' + JSON.stringify(invoiceDocNo) + '. Inititated Multi-Vehicle E-Waybill Group Number is - ' + groupNumber;
