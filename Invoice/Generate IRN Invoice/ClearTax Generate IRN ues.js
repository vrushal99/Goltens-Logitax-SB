...
*/
define(["N/record","N/runtime", "N/search", "N/ui/serverWidget", 'N/url','./ClearTax Library File.js'], function(record, runtime, search, serverWidget, url,ClearTax_Library) {
		...
		try {
			...
			if (typeOdMode == "view") // only it will execute "view" mode
			{
				...
				if (_logValidation(record_obj)) {
					...
					form.clientScriptModulePath = './ClearTax Generate IRN cli.js';
				}	
			}
		} catch (ex) {
			log.debug({
