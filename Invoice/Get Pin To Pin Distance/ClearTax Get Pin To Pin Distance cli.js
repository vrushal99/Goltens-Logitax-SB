/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Pin To Pin Distance cli
        Author 			:  	NVT Employee
        Date            :   10-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_Pin_To_Pin_Distance() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_get_pin_dist_sui",
                    deploymentId: "customdeploy_cleartax_get_pin_dist_sui"
                });
        ...
        log.error('error in ClearTax_Get_Pin_To_Pin_Distance() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_Pin_To_Pin_Distance: ClearTax_Get_Pin_To_Pin_Distance
}
});