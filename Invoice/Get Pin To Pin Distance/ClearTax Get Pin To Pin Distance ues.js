/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Get Pin To Pin Distance ues
		Author 			:  	NVT Employee 
		Date            :   10-07-2024
		Description		:	
				...
				if (_logValidation(record_obj)) {
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_irn)) { //if cleartax einvoice irn present
							...
							form.addButton({
								id: 'custpage_get_pin_distance',
								label: 'Get Pin To Pin Distance',
								functionName: "ClearTax_Get_Pin_To_Pin_Distance"
							});
					...
					form.clientScriptModulePath = './ClearTax Get Pin To Pin Distance cli.js';
				}
			}
		} catch (ex) {
