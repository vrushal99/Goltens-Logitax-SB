/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Eway Bill by Number cli
        Author 			:  	NVT Employee
        Date            :   08-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_Ewaybill_By_Number() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_get_ewb_by_num_sui",
                    deploymentId: "customdeploy_cleartax_get_ewb_by_num_sui"
                });
        ...
        log.error('error in ClearTax_Get_Ewaybill_By_Number() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_Ewaybill_By_Number: ClearTax_Get_Ewaybill_By_Number
}
});