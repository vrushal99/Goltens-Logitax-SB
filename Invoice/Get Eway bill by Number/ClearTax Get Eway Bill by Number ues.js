/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Get Eway Bill by Number ues
		Author 			:  	NVT Employee 
		Date            :   08-07-2024
		Description		:	
				...
				if (_logValidation(record_obj)) {
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_irn) && _logValidation(ctax_ewaybill_number) && ctax_ewaybill_status != 2) { //if cleartax einvoice irn present and ewaybill status is not equal to cancel and ewaybill number not empty then button will display
							...
							form.addButton({
								id: 'custpage_ewaybill_by_number',
								label: 'Get Eway Bill By Number',
								functionName: "ClearTax_Get_Ewaybill_By_Number"
							});
					...
					form.clientScriptModulePath = './ClearTax Get Eway Bill by Number cli.js';
				}	
			}
		} catch (ex) {
