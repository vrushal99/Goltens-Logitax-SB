/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Eway Bill by Number sui
        Author 			:  	NVT Employee
        Date            :   08-07-2024
        Description		:   
    ...
    function (serverWidget, search, file, encode, format, url, config, xmlobj, render, record, https, redirect, runtime, ClearTax_Library_File) {
        function onRequest(context) {
            try {
                ...
                var ctax_ewaybill_number = loadRecord.getValue({
                ...
                var get_ewaybill_by_num_url = get_environment_name["GET_EWAYBILL_BY_NUMBER_URL"]
                log.debug("get_ewaybill_by_num_url", get_ewaybill_by_num_url)
                ...
                var url = get_ewaybill_by_num_url;
                //log.debug("url", url)
                var headers = {
                ...
                loadRecord.setValue({
                    fieldId: 'custbody_logitax_get_eway_bill_request',
                ...
                loadRecord.setValue({
                    fieldId: 'custbody_logitax_get_eway_bill_number',
