/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Eway Bil details by IRN sui
        Author 			:  	NVT Employee
        Date            :   01-07-2024
        Description		:   
    ...
    function (serverWidget, search, file, encode, format, url, config, xmlobj, render, record, https, redirect, runtime, ClearTax_Library_File) {
        function onRequest(context) {
            try {
                ...
                var get_ewaybill_by_irn_url = get_environment_name["GET_EWAYBILL_BY_IRN_URL"]
                log.debug("get_ewaybill_by_irn_url", get_ewaybill_by_irn_url)
                ...
                var url = get_ewaybill_by_irn_url;
                //log.debug("url", url)
                var headers = {
                ...
                loadRecord.setValue({
                    fieldId: 'custbody_ctax_get_ewb_irn_request',
                ...
                loadRecord.setValue({
                    fieldId: 'custbody_ctax_get_ewb_irn_response',
