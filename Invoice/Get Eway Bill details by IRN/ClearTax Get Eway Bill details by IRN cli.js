/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Eway Bil details by IRN cli
        Author 			:  	NVT Employee
        Date            :   01-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_Ewaybill_By_IRN() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_ewaybill_by_irn_su",
                    deploymentId: "customdeploy_cleartax_ewaybill_by_irn_su"
                });
        ...
        log.error('error in ClearTax_Get_Ewaybill_By_IRN() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_Ewaybill_By_IRN: ClearTax_Get_Ewaybill_By_IRN
}
});