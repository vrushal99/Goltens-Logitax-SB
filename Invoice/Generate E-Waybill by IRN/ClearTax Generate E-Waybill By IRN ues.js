/*----------------------------------------------------------------------------------------------
		...
		Script Name 	:	ClearTax Generate E-Waybill By IRN ues
		Author 			:  	NVT Employee 
		Date            :   01-07-2024
		Description		:	
		...
		try {
			...
			if (typeOdMode == "view") // only it will execute "view" mode
			{
				...
				if (_logValidation(record_obj)) {
					...
					var ctax_einvoice_cancel_irn_date = record_obj.getValue({
					...
					if (getInvStatus !== "Voided") {
						...
						if (_logValidation(ctax_einvoice_irn) && ctax_ewaybill_check == false && !_logValidation(ctax_einvoice_cancel_irn_date)) { //if cleartax einvoice irn present and ewaybill transfer is checked then button will display
						...
						else if (_logValidation(ctax_einvoice_irn) && ctax_ewaybill_check == true && ctax_ewaybill_status == 2 && !_logValidation(ctax_einvoice_cancel_irn_date)) { //if invoice irn present and ewaybill transfer checkbox is checked and ewaybill status is cancel.
					...
					form.clientScriptModulePath = './ClearTax Generate E-Waybill By IRN cli.js';
				}	
			}
		} catch (ex) {
			log.debug({
