/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Transporter Details cli
        Author 			:  	NVT Employee
        Date            :   10-07-2024
        Description		:	
------------------------------------------------------------------------------------------------*/
...
define(["N/record", "N/currentRecord", "N/format", 'N/search', 'N/ui/message', 'N/url'], function (record, currentRecord, format, search, message, url) {
    ...
    function ClearTax_Get_Transporter_Details() {
        try {
            ...
            if (nullCheck(recordId) && nullCheck(recordType)) {
                ...
                var s_sut_url = url.resolveScript({    // call suitelet script
                    scriptId: "customscript_cleartax_get_transp_det_sui",
                    deploymentId: "customdeploy_cleartax_get_transp_det_sui"
                });
        ...
        log.error('error in ClearTax_Get_Transporter_Details() function', e.toString());
    ...
    return {
    pageInit: pageInit,
    ClearTax_Get_Transporter_Details: ClearTax_Get_Transporter_Details
}
});