/*----------------------------------------------------------------------------------------------
        ...
        Script Name 	:	ClearTax Get Transporter Details sui
        Author 			:  	NVT Employee
        Date            :   10-07-2024
        Description		:   
    ...
    function (serverWidget, search, file, encode, format, url, config, xmlobj, render, record, https, redirect, runtime, ClearTax_Library_File) {
        function onRequest(context) {
            try {
                ...
                var get_transporter_details_url = get_environment_name["GET_TRANSPORTER_DETAILS_URL"]
                log.debug("get_transporter_details_url", get_transporter_details_url)
                ...
                var url = get_transporter_details_url;
                log.debug("url", url)
                ...
                loadRecord.setValue({
                    fieldId: 'custbody_logitax_transported_request',
                ...
                loadRecord.setValue({
                    fieldId: 'custbody_logitax_transporter_response',
