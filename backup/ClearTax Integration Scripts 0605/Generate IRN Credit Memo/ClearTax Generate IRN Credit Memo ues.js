/*----------------------------------------------------------------------------------------------
		...
		Description		:	1. This Script is generated for creditmemo to ClearTax system.
		                       E-creditmemo generation button is created through this script and it is invoked only for regular and GST                                   registered Users.
		...
		try {
			...
			if (typeOdMode == "view") 
			{
					...
					if (_logValidation(record_obj)) 
					{
                     ...
                     form.clientScriptModulePath = '/SuiteScripts/ClearTax Integration Scripts/ClearTax Generate IRN Credit Memo cli.js';	 // this is path is call client script				
			}
		} catch (ex) {
			log.debug({
				title: 'ex:',
