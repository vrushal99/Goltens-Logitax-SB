...
define(['N/ui/serverWidget', 'N/search', 'N/file', 'N/encode', 'N/format', 'N/url', 'N/config', 'N/xml', 'N/render', 'N/record', 'N/https', 'N/redirect', 'N/runtime', '/SuiteScripts/ClearTax Integration Scripts/ClearTax Library File.js'],
    function (serverWidget, search, file, encode, format, url, config, xmlobj, render, record, https, redirect, runtime, ClearTax_Library_File) {
        function onRequest(context) {
			...
			var shipping_obj = shippingAdressDetails(recID)
			...
			if (nullCheck(exportCountryCode)) {
				...
				var ewaybll_generate_irn_url = get_environment_name["GENERATE_IRN_URL"]
				log.debug("ewaybll_generate_irn_url",ewaybll_generate_irn_url)
			...
			alert("Invoice to ClearTax");
			
			// var url = 'https://api-sandbox.clear.in/einv/v2/eInvoice/generate'; // static value
			var url = ewaybll_generate_irn_url;
			//log.debug("url", url)
			var headers = {
				"X-Cleartax-Auth-Token": auth_token, // static value
				// "owner_id": '9abeb2db-955c-49c6-99b9-e2589314896b', // static value //05.04.2024 - commented
				"gstin": subsidiary_obj_gstnum,
				// "x-cleartax-product": 'EInvoice', // static value //05.04.2024 - commented
			...
			if (response_irn.code == 200) {  //if response code got 200 then this code will execute
				...
				if (nullCheck(error) && error == "N") {  //if response code got 200 and success got 'N' then this code will execute
					...
					alert(get_error)  // what is error we are getting this alert will print
					...
					log.debug("parseObdy_AckNo",parseObdy_AckNo)
					var parseObdy_AckNo_str = parseObdy_AckNo.toString();
					log.debug("parseObdy_AckNo_str",parseObdy_AckNo_str);
					...
					// alert("E-invoice generate successfully.")
				}
