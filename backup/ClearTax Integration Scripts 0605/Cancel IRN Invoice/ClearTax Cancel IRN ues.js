...
define(['N/record', 'N/file', 'N/config', 'N/xml', 'N/search', 'N/render', 'N/format', 'N/task', 'N/http', 'N/runtime', 'N/https', '/SuiteScripts/ClearTax Integration Scripts/ClearTax Library File.js'],
	function (record, file, config, xml, search, render, format, task, http, runtime, https, ClearTax_Library_File) {
		...
		function beforeLoad_ClearTax_Cancel_IRN_ues(context) {
			try {
				if (context.type == context.UserEventType.EDIT) {
					...
					var invoice_status = invoice_obj.getValue({
						fieldId: 'status'
					});
					...
					if (_logValidation(invoice_ack_date)) {  // if ack_date have then condition will execute
						...
						if (!_logValidation(invoice_irn)) { //voidfix:16.04.2024 - add condition to remove button if irn is not presen
							...
							if (invoice_irn && invoice_ack_date && (d_date.getTime() < todaytime)) {  // if invoice_irn, invoice_ack_date is todaytime greterthan 24 hour then button will remove
								Formobj.removeButton({
									id: "void"
								})
							}
		...
		function beforeSubmit_ClearTax_Cancel_IRN_ues(context) {
			...
			try {
				...
				if (context.type == context.UserEventType.EDIT) {
					...
					var cancelReasonCode = invoice_obj.getValue({
						fieldId: 'custbody_ctax_einvoice_cancel_irn'
					});
					log.debug('cancelReasonCode', cancelReasonCode);

					var cancelRemark = invoice_obj.getValue({
						fieldId: 'custbody_ctax_cancel__irn_remark'
					});
					log.debug('cancelRemark', cancelRemark);

					if (!_logValidation(cancelReasonCode) && !_logValidation(cancelRemark)) {

						throw "Please select CANCEL IRN reason code and CANCEL REMARK.";
				...
				log.debug({ title: "err", details: e.toString() });
				throw e;
			}
		}
		...
		return {
			...
			//beforeSubmit: beforeSubmit_ClearTax_Cancel_IRN_ues,
