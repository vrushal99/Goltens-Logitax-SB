		...
		try {
			...
			if (typeOdMode == "view") // only it will execute "view" mode
			{
					...
					if (_logValidation(record_obj)) 
					{
						...
						if(_logValidation(ctax_einvoice_irn) && ctax_ewaybill_check == false){ //if cleartax einvoice irn present and ewaybill transfer is checked then button will display
                    ...
                    form.clientScriptModulePath = '/SuiteScripts/ClearTax Integration Scripts/ClearTax Generate E-Waybill By IRN Credit Memo cli.js';					
			}
		} catch (ex) {
			log.debug({
				title: 'ex:',
